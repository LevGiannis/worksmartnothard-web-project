...
**Migration FTTH** (`Κωδ. Χρήστη` header, ξεχωριστό αρχείο Excel από το Vodafone Home): κρατά μόνο γραμμές όπου η `Επιλεγμένη Ταχύτητα` περιέχει `FTTH` **και** η `Ταχύτητα πριν το Retention` δεν περιέχει `FTTH` — δηλαδή μόνο πραγματικές μεταβάσεις σε FTTH. Δεν επηρεάζει κανένα άλλο σύνολο/export (`isDone` δεν το αναγνωρίζει ως done)· μετράει αποκλειστικά στο παράθυρο "Migration FTTH — Μετράνε στον Μήνα": αιτήσεις υποβεβλημένες (`Ημ/νια Υποβολής Αίτησης στο Fixed Siebel`, fallback στην `Ημ/νια Δημιουργίας Αίτησης`) τον μήνα με status `ΥΛΟΠΟΙΗΜΕΝΗ` ή `ΥΠΟ ΥΛΟΠΟΙΗΣΗ`.
